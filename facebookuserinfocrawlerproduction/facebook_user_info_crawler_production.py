...
from Create_Proxy import RequestProxy
...
def crawler_config_and_login_account(_no):
	...
	#create browser
	browser = webdriver.Chrome(desired_capabilities=capabilities, executable_path = config.CHROME_DRIVER_PATH, chrome_options = chrome_options)
...
def reset(browser_1, index, _no):
	...
	#create browser
	browser = webdriver.Chrome(executable_path = config.CHROME_DRIVER_PATH, chrome_options = chrome_options, desired_capabilities=capabilities)
	browser.get(config.URL)
	#login into account
	k1 = None
	k2 = None
