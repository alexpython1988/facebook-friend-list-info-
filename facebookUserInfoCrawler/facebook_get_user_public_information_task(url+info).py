...
queue = My_Queue()
page_bottom = [
    "medley_header_events",
    "medley_header_photos",
    "medley_header_likes"]
FORMAT = '%(asctime)-20s %(name)-5s %(levelname)-10s %(message)s'
logging.basicConfig(
    filename='get_user_info.log',
    level=logging.DEBUG,
    format=FORMAT,
    datefmt="%Y-%m-%d %H:%M:%S")
...
def scrapy_user_info(browser, uid, person_info):
    ...
    for index in range(2, 7):
        browser.find_element_by_xpath(
            "//ul[@class='uiList _4kg']/li[{}]/a[@class='_5pwr']".format(index)).click()
        time.sleep(1)
...
def get_info_of_user(browser, index):
    ...
    if index == 2:
        ...
        for each_div_2_1 in divs:
            # section title
            title_text_2_1 = each_div_2_1.find(
                "div", class_="clearfix _h71").text
            l2 = []
            # section content
            ...
            if (divs_2_2 is None) or (len(divs_2_2) == 0):
                ...
                for each_div_2_2 in divs_2_2:
                    c2 = dict()
                    place_info_2 = each_div_2_2.find(
                        "div", class_="_2lzr _50f5 _50f7").text
                    details_2 = each_div_2_2.find(
                        "div", class_="fsm fwn fcg").text
        ...
        if num == 0:
            ...
            if len(divs_3_1a) > 0:
                for each_div_3_1a in divs_3_1a:
                    c3 = dict()
                    div_3_1aa = each_div_3_1a.find(
                        "span", class_="_50f5 _50f7")
            ...
            if (divs_3_1b is None) or (len(divs_3_1b) == 0):
                ...
                for each_div_3_1b in divs_3_1b:
                    c3 = dict()
                    loc_info_3_1b = each_div_3_1b.find(
                        "span", class_="_50f5 _50f7").text
            ...
            if (divs_3_2 is None) or (len(divs_3_2) == 0):
                ...
                for each_div_3_2 in divs_3_2:
                    c3 = dict()
                    loc_info_3_2 = each_div_3_2.find(
                        "span", class_="_50f5 _50f7").text
        ...
        for each_div_4_1 in divs:
            # each div title
            title_text_4_1 = each_div_4_1.find(
                "div", class_="clearfix _h71").text
            ...
            if (divs_4_2 is None) or (len(divs_4_2) == 0):
                ...
                for each_div_4_2 in divs_4_2:
                    key = each_div_4_2.find(
                        "div", class_="_4bl7 _3xdi _52ju").text
        ...
        for i, each_div_5_1 in enumerate(divs):
            l5 = []
            # each div title
            title_text_5_1 = each_div_5_1.find(
                "div", class_="clearfix _h71").text
            ...
            for each_div_5_2 in divs_5_1:
                ...
                if len(a_5_1) == 0:
                    if i == 0:
                        ...
                        u_name = each_div_5_2.find(
                            "span", class_="_50f5 _50f7").text if each_div_5_2.find(
                            "span", class_="_50f5 _50f7") is not None else None
                        u_relation = each_div_5_2.find_all(
                            "div", class_="fsm fwn fcg")[1].text if len(
                            each_div_5_2.find_all(
                                "div", class_="fsm fwn fcg")) > 1 else None
                        c5["name"] = u_name
                ...
                # 		l5.append(c5)
                    ...
                    u_relation = None
                    if i == 0:
                        u_relation = each_div_5_2.find(
                            "div", class_="_173e _50f8 _50f3").text
                    elif i == 1:
                        u_relation = each_div_5_2.find_all(
                            "div", class_="fsm fwn fcg")[1].text
                    # add info with title
                    c5["name"] = u_name
                    c5["user_id"] = u_id
                    c5["relationship"] = u_relation
        ...
        for each_div_6_1 in divs:
            l6 = []
            title_text_6_1 = each_div_6_1.find(
                "div", class_="clearfix _h71").text
            ...
            for each_class in class_6:
                ...
                if u_info_div is not None:
                    if each_class == "_2tdc":
                        for each_div_6_2 in each_div_6_1.find_all(
                                "li", class_="_43c8 _5f6p _3twh"):
                            u_info = dict()
                            value = each_div_6_2.find(
                                "span", class_="_50f4").text
                            key_div = each_div_6_2.find(
                                "div", class_="fsm fwn fcg")
...
def scrapy_friend_list_of_friends(browser, person_info):
    time.sleep(0.2)
    browser.find_element_by_xpath(
        "//div[@class='_6_7 clearfix lfloat _ohe']/a[3]").click()
...
def scrapy_friend_list_based_on_account(browser, person_info):
    ...
    ul_list = ul + ul_ext
    # find all links for each friend in friend list in order to continue
    # obtain information of their friends
        ...
        for each_div in divs:
            ...
            for each_a in a_list:
                ...
                if(href_info.endswith("_tab")):
                    ...
                    # use this as key to distinguish if two people with same
                    # name is the same person
                    id_url = href_info.split("/")[-1]
                    fid = ""
                    if(id_url.startswith("profile.php")):
                        fid += id_url.split("?")[-1].split("&")[
                            0].split("=")[-1]
                    else:
                        fid += id_url.split("?")[0]
